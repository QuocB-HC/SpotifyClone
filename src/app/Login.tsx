...
const LoginScreen = () => {
  ...
  async function onGoogleButtonPress() {
    try {
      ...
      // console.log("Thông tin người dùng:", userInfo.data?.user);
      // console.log("idToken:", idToken);
      const userSnapshot = await firestore()
        .collection("users")
        .where("email", "==", userInfo.data?.user.email)
        .get();
      ...
      if (userSnapshot.empty) {
        try {
          // Tham chiếu đến collection "user"
          const userCollection = firestore().collection("users");

          // Thêm dữ liệu vào Firestore
          const response = await userCollection.add(userData);

          console.log("User added with ID:", response.id);
        } catch (error) {
          console.error("Error adding user to Firestore:", error);
        }
      } else {
        console.log("User", userInfo.data?.user.name, "đã tồn tại !")
      }
